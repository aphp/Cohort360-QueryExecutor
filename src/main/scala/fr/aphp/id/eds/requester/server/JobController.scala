...
import fr.aphp.id.eds.requester.jobs.{JobManager, JobResultSerializer, JobType, JobsConfig, SparkJobParameter}
...
class JobController(implicit val swagger: Swagger) extends ScalatraServlet with SwaggerSupport with JacksonJsonSupport {
  ...
  private def parseInput(body: String): SparkJobParameter = {
    ...
    SparkJobParameter(
      ...
      if (configData.hasPath("callbackPath")) Option(configData.getString("callbackPath")) else Option.empty,
